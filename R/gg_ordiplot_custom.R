...
gg_ordiplot_egc <- function(ord, groups, scaling = 1, choices = c(1,2), kind = c("sd", "se", "ehull"), conf=NULL, show.groups="all", ellipse = TRUE, label = FALSE, hull = FALSE, spiders = FALSE, pt.size = 3, plot=TRUE) {
  ...
  xlab <- axis.labels[1]
  ylab <- axis.labels[2]
    plt <- df_ord %>% mutate(group2=ifelse(Group %in% c("Oysterville","Long Beach"), "Clam Bed", "Slough")) %>%
      ggplot2::ggplot() +
      geom_point(aes(x=x, y=y, pch=group2, color=Group), size = pt.size) +
      xlab(xlab) + ylab(ylab)
  ...
  # Add hulls.
  if (hull == TRUE) {
    plt <- plt + geom_path(data=(df_hull %>% mutate(Group2=ifelse(Group %in% c("Oysterville","Long Beach"), "Clam Bed", "Slough"))), aes(x=x, y=y, color=Group2), show.legend = FALSE)
  }
  
  # Add spiders.
  if (spiders == TRUE) {
    # plt <- plt + geom_segment(data=df_spiders %>% mutate(Group2=ifelse(Group %in% c("Oysterville","Long Beach"), "Clam Bed", "Slough")), aes(x=cntr.x, xend=x, y=cntr.y, yend=y, color=Group2, linetype=Group), show.legend = FALSE)
    plt <- plt + geom_segment(data=df_spiders %>% mutate(Group2=ifelse(Group %in% c("Oysterville","Long Beach"), "Clam Bed", "Slough")), aes(x=cntr.x, xend=x, y=cntr.y, yend=y, color=Group, linetype=Group2), show.legend = FALSE)
  }
