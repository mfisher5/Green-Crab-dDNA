...
```{r}
QM_Bayes_filter <- read_csv(here('../../','data','qm_model','QM_Bayes_posterior_means_filtered_08-29-2023.csv'))
...
```{r}
all_dDNA_data_Observation <- readRDS(here('../../','data','qm_model','QM_Bayes_my_dDNA_data_observation.rds'))
```
...
```
how different are the results for the those samples?
```{r}
...
png(here('zoid_input_Bayes_est.png'),res=300,height=1500,width=1500)
...
saveRDS(bayes_wide,here('../../','data','zoid','zoid_input_Bayes_est_rmultinomReads_20230829.rds'))
...
```{r}
png(here('zoid_results_rmultinomReads_20230829.png'), res=300,height=1500,width=1500)
...
```{r}
write_csv(fitted_vals_1, here('../../','data','zoid','zoid_fitted_vals_preyD100_2023-08-29.csv'))
```



# Run Zoid : by site type
...
```{r}
png(here('zoid_results_CLAMBED_rmultinomReads_20230829.png'), res=300,height=1500,width=1500)
avg_crab1 + theme(plot.margin=margin(5.5,5.5,5.5,20, unit="pt"))
dev.off()
```
...
Save fitted vals! use these later to make the combined figure 
```{r}
write_csv(fitted_vals_clambed, here('../../','data','zoid','zoid_fitted_vals_CLAMBED_preyD100_2023-08-29.csv'))
```
...
Save fitted vals! use these later to make the combined figure 
```{r}
write_csv(fitted_vals_slough, here('../../','data','zoid','zoid_fitted_vals_SLOUGH_preyD100_2023-08-29.csv'))
```
...
saveRDS(zoid.list, here('../../','data','zoid','zoidOUT_preyD100_2023-08-29.rds'))
```
