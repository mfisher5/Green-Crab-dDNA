...
get the hashes in the controls. **Controls not run through blast for run 5**
...
control_dat <- blast.dat.filter %>% filter((source=="Control" & taxon != "Carcinus maenas") | taxon %in% c("Homo sapiens","Primates"))
```
<br>
...
blast.dat.filter <- blast.dat.filter %>% 
  mutate(baf_rm=ifelse(predator_rm==0 & (phylum %in% to_remove_phyla | taxon %in% to_remove_taxa), 1, 0))
``` 
<br>
<br>
...
to_remove_taxa  <- c("uncultured Bangiophyceae","Tetraselmis","Chromulinales","Chloropicon","Chloropicon primus","Chloroparvula pacifica","	Paraphysomonas sp.","uncultured Jiaozhou Bay zooplankton","Trebouxia aggregata","Lobosphaera incisa",	"Scenedesmus quadricauda","Trebouxiophyceae sp. M-hakoo 311","Pseudoralfsia verrucosa","Thecamonas trahens","Cyprideis torosa","Gallus gallus","Myrionema strangulans","Lobosphaera incisa","Trebouxiophyceae sp. M-hakoo 311","Bathycoccus prasinos","	Paraphysomonas sp.","uncultured organism","Fungi","uncultured fungus")
to_remove_genus <- c("Scomber")

blast.dat.filter <- blast.dat.filter %>% 
  mutate(too_small_rm=ifelse(baf_rm==0 & (phylum %in% to_remove_phyla | taxon %in% to_remove_taxa | genus %in% to_remove_genus), 1, 0))
```
<br>
