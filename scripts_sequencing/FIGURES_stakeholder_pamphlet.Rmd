...
### % crab per species
```{r}
unqdat_sites %>% group_by(rank,taxon,taxon.corrected) %>%
...
```{r}
unqdat_sites_sp <- unqdat_sites %>% group_by(site,rank,taxon,taxon.corrected,phylum,class,order,family,taxon.group) %>%
  summarise(n.crab=length(unique(sample))) %>%
  left_join(unqdat_sites %>% 
              group_by(site) %>% 
...
plotdat <- unqdat_sites_sp %>% filter(order %in% c("Decapoda","Alismatales") | class %in% c("Bivalvia","Actinopteri")) %>%
  rename(`Percent of Crabs\nwith food item`=p.crab) %>%
  filter(!(grepl("Crangon",taxon.corrected)) & taxon != "Zosteraceae") %>%
  mutate(site_type=ifelse(site %in% c("Long Beach","Oysterville"), "Clam Bed", "Monitoring"))

plotdat <- plotdat %>% mutate(taxon2=ifelse(taxon.corrected=="Mya arenaria","Soft-shell clam",
                                                   ifelse(taxon.corrected=="Crangon franciscorum", "Sand shrimp",
                                                          ifelse(taxon.corrected=="Hemigrapsus oregonensis", "Hairy shore crab",
                                                                ifelse(taxon.corrected=="Leptocottus armatus","Pacific Staghorn\nsculpin",
                                                                       ifelse(taxon.corrected=="Zosteraceae","Surfgrass/Eelgrass",
                                                                              ifelse(taxon.corrected=="Metacarcinus magister", "Dungeness crab",
                                                                                     ifelse(taxon.corrected=="Cymatogaster aggregata","Shiner perch",
                                                                                            ifelse(taxon.corrected=="Kurtiella sp.","Small clam\n(Kurtiella sp.)",ifelse(taxon.corrected=="Pholis ornata","Saddleback gunnel",ifelse(taxon.corrected=="Ruditapes philippinarum","Manila clam",NA)))))))))))

plotdat$taxon2 <- factor(plotdat$taxon2, levels=rev(c("Soft-shell clam","Manila clam","Small clam\n(Kurtiella sp.)",
                                                  "Pacific Staghorn\nsculpin","Shiner perch","Saddleback gunnel","Hairy shore crab","Dungeness crab","Cordgrasses/Eelgrasses")))
...
ggplot(plotdat) +
  geom_point(aes(x=taxon2,y=site,size=`Percent of Crabs\nwith food item`, col=site)) + 
  geom_point(aes(x=taxon2,y=site,size=`Percent of Crabs\nwith food item`), pch=21) +
  geom_rect(aes(xmin=6.5,xmax=7.5,ymin=1.75,ymax=2.25), col="gray60",fill=NA) +
  geom_segment(aes(x=7,xend=6.80,y=2.25,yend=2.6), col="gray60") +
  geom_text(data=data.frame(x=6.8,y=3.12,label="1 green crab"), aes(x=x,y=y,label=label), col="gray60", fontface="italic",size=2) +
...
```
```{r}
png(here('results','figures','pamphlet','prey-clams-crab-fish_by-site_CEGblast_pamphlet.png'),res=300,height=1500,width=1500,bg = 'transparent')
ggplot(plotdat) +
  geom_point(aes(x=taxon2,y=site,size=`Percent of Crabs\nwith food item`, col=site)) + 
  geom_point(aes(x=taxon2,y=site,size=`Percent of Crabs\nwith food item`), pch=21) +
  geom_rect(aes(xmin=6.75,xmax=7.25,ymin=1.75,ymax=2.25), col="gray60",fill=NA) +
  geom_segment(aes(x=7,xend=6.80,y=2.25,yend=2.6), col="gray60") +
  geom_text(data=data.frame(x=6.8,y=3.5,label="1 green crab"), aes(x=x,y=y,label=label), col="gray60", fontface="italic",size=3) +
  scale_color_manual(values=col_palette2,name="") +
  scale_size_continuous(range=c(2,6)) +
  guides(color=FALSE) +
