...
```{r echo=FALSE, fig.height=3, fig.width=8, eval=TRUE}
...
to_remove_family <- c("Chrysophyceae")

blast.dat.filter <- blast.dat.filter %>% 
  mutate(too_small_rm=ifelse(control_rm==0 & (phylum %in% to_remove_phyla | taxon %in% to_remove_taxa | genus %in% to_remove_genus) | family %in% to_remove_family, 1, 0))
```
<br>
...
```{r echo=FALSE, fig.height=5, fig.width=10, eval=TRUE}
site <- "Nahcotta"
ggplot(plotdat %>% filter(grepl(site, site_month)), aes(x=sample_id,y=pReads,fill=reason)) +
  geom_col() +
  facet_grid(cols=vars(sample), scales="free") +
