...
blast_file <- 'run2_bf3_sample_CEGblast_lca_taxonomy.csv'
```
<br>

What is the location / name of the metadata file for this sequencing run?
```{r}
run_metadat_file <- 'data/metadata/run2_BF3_filtered_metadata.csv'
```
<br>
...
control_dat <- blast.dat.filter %>% filter((type=="Control" & taxon != "Carcinus maenas") | taxon %in% c("Homo sapiens","Primates"))
```
<br>
...
blast.dat.filter <- blast.dat.filter %>% 
  mutate(too_small_rm=ifelse(control_rm==0 & (phylum %in% to_remove_phyla | taxon %in% to_remove_taxa | genus %in% to_remove_genus) | class %in% to_remove_class, 1, 0))
```
<br>
